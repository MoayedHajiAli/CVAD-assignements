...
class AffordancePredictor(nn.Module):
    ...
    def __init__(self, n_commands=4):
        ...
        self.backbone.append(nn.Conv2d(2048, 2048, kernel_size=3, padding=1))
        ...
        self.uncond_head = nn.Sequential(nn.Linear(2048, 512), nn.ReLU(), nn.Dropout(0.5),
                             nn.Linear(512, 256), nn.ReLU(), nn.Linear(256, 2))
        
        # cond: the afford
        self.con_head = nn.ModuleList([ nn.Sequential(nn.Linear(2048, 512), nn.ReLU(),
